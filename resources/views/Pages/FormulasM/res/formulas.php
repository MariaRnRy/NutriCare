<?php
	...
	function imc($q, $p) //se puede utilizar la funcion de clasif()
	{
	...
	function MP($q, $p) //No requiere de dos servicios
	{
	...
	function PA($q, $p) //No requiere de dos servicios
	{ 
		if ($q <= 787.9) {
	...
	function pesoInfe($q, $p) //No requiere de dos servicios
	{ 
		$z = $q * $q * 18.5;
	...
	function pesoSupe($q, $p) //No requiere de dos servicios
	{
	...
	function pesoTeo($q, $p) //No requiere de dos servicios
	{
	...
	function ppci($q, $p)
	{
		...
		elseif (80 <= $z && $z <= 90) {
			...
			echo "SIN DEPLECION";_format($z, 2, '.', ',')." Kg";
			}
	function PPH($q, $p)
	{
			  $z = ($q/$p)*100;
	...
	function PPT($q, $p)//Tambien se puede usar la funcion de ppci
	{
		     $w = $p * $p * 21.5;
		    $z = ($q/$w)*100;
